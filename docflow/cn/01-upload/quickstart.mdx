...
<Tip>
本文演示如何通过 REST API 上传文件到 DocFlow。更多支持的文件格式见《[文件格式支持](./support_format.mdx)》。
</Tip>
...
- `workspace_id`: 空间ID。可以参考[此文档](../00-overview/quickstart#%E9%85%8D%E7%BD%AEdocflow%E7%A9%BA%E9%97%B4%E4%B8%8E%E5%88%86%E7%B1%BB)获取。
...
示例：
<CodeGroup>
```python Python icon=python expandable lines highlight={35-50}
import requests
import json
from requests_toolbelt.multipart.encoder import MultipartEncoder
import time
