...
当在 DocFlow 空间设置页面配置好文件分类模板后(**TODO: 说明如何在页面上配置**)，后续文件上传时，如果不指定该文件类型，Docflow 都会自动对文件进行分类，以便后续抽取或审核使用。
...
如果只想要分类结果，不想要抽取结果(为了减少时间开销)，可以在上传接口中加入`target_process=classify`参数，流程就会在完成分类后终止，跳过抽取过程。
</Tip>
