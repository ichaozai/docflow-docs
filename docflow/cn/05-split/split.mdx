---
title: "拆套"
description: "自动拆分长文档为单页或子文档，支持多页文件按内容类型智能拆分"
---

## 功能概述

拆套功能能够自动识别多页文档中的不同内容类型，并将其拆分为独立的子文档。这对于处理包含多种文档类型的复合文件非常有用。
...
通过拆套功能，可以将这三种不同类型的文档分别拆分出来，便于后续的分类和抽取处理。
...
拆套功能能够将这些不同类型的单据分别识别和拆分。

<iframe src="../images/split_example.pdf" width="100%" height="600px" />
...
在上传接口中设置 `split_flag=true` 来启用拆套功能：
...
| 参数名 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `split_flag` | boolean | false | 是否启用拆套功能 |
...
if __name__ == "__main__":
    ...
    if batch_number:
        ...
        print("拆套结果:", json.dumps(fetch_result, indent=2, ensure_ascii=False))
```
...
当启用拆套功能后，`file/fetch` 接口返回的结果中会包含 `child_files` 字段，用于描述拆分后的子文档信息：
...
| `child_files[].task_type` | integer | 任务类型，0表示拆套产生 |
