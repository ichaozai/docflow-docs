...
curl -X POST \
  ...
  "https://docflow.textin.com/api/app-api/sip/platform/v2/category/fields/batch_add"
```
...
result = resp.json()
if result.get("code") == 200:
    field_ids = result.get("result", {}).get("field_ids", [])
    print(f"字段创建成功，IDs: {field_ids}")
else:
...
if result.get("code") == 200:
    field_ids = result.get("result", {}).get("field_ids", [])
    print(f"表格字段创建成功，IDs: {field_ids}")
else:
    print(f"创建失败: {result.get('msg')}")
```
...
curl -X POST \
  ...
  "https://docflow.textin.com/api/app-api/sip/platform/v2/category/fields/batch_update"
```
...
- `fields` (必填): 字段数组，每个元素包含以下属性：
  - `field_id` (必填): 字段 ID
  - 其他属性与创建字段相同（name、description、prompt 等）
