...
**要求**：每个文件类别至少需要 1 个样本文件，最多 20 个，建议上传 3-5 个具有代表性的样本以获得最佳效果。
...
curl -X POST \
  ...
  -F 'category_keyword_rules={"positive_rules":[{"group_name":"发票关键字","min_hit":2,"words":["发票代码","发票号码"]}],"negative_rules":[]}' \
...
- `extract_model` (必填): 抽取模型，可选值：`Auto`、`Acgpt`、`Acgpt-VL`、`DF-M1`
- `sample_files` (必填): 样本文件列表，至少上传一个样本文件；每个类别最多拥有20个样本文件
- `fields` (必填): 字段配置列表（JSON 字符串），至少配置一个字段，表格字段仅允许配置默认表格中的字段（table_id=-1）
- `category_keyword_rules` (选填): 用于分类的关键字规则（JSON 字符串），正向规则用于匹配，负向规则用于过滤。正向规则最多配置 10 组，负向规则最多配置 1 组，每组最多支持配置 20 个关键字

<Note>
  同一规则组中至少命中 **min\_hit** 个关键字时，该规则组命中。多个正向规则组之间是 OR 关系；命中反向规则组时排除该类别。创建时不传或传入两个空数组表示不配置规则。
</Note>
...
if result.get("code") == 200:
    ...
    for category in categories:
        ...
        print(f"分类关键字规则: {category.get('category_keyword_rules')}")
else:
...
已配置分类关键字的类别会在 `category_keyword_rules` 中返回完整规则对象，可用于后续展示或更新。
...
- `category_prompt` (选填): 用于分类的提示词，最大长度 500
- `category_keyword_rules` (选填): 用于分类的关键字规则，正向规则用于匹配，负向规则用于过滤。正向规则最多配置 10 组，负向规则最多配置 1 组，每组最多支持配置 20 个关键字。不传或传 `null` 时不修改，传有效对象时全量替换，传 `{"positive_rules":[],"negative_rules":[]}` 时清空规则
