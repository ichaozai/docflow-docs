...
<Tip>
  样本文件是文件类别的示例文档，用于训练和优化文档分类与抽取模型。每个文件类别至少需要一个样本文件，最多 20 个样本文件。本文介绍如何通过 API 管理文件类别的样本文件。
</Tip>
...
- `files` (必填): 样本文件，单次请求最多上传 20 个；上传后该类别的样本总数不能超过 20 个，因此实际可上传数量为 `20 - 当前已有样本数`
...
<Note>
  **样本文件要求**：
  ...
  - 每个类别最多保留 20 个样本；追加上传时请扣除该类别已有样本数
  - 支持的文件格式参考文档上传支持的格式
</Note>
