...
After configuring classification sample files in Docflow, the sample files are first parsed, then the title, file type, field configuration, classification prompts, etc. of the sample files are vectorized and analyzed.

After new files are uploaded and parsing is completed, algorithms are used to compare the new files with configured classifications to obtain the most matching classification results.
