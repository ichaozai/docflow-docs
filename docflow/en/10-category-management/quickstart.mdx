...
curl -X POST \
  ...
  -F 'category_keyword_rules={"positive_rules":[{"group_name":"Invoice keywords","min_hit":2,"words":["invoice code","invoice number"]}],"negative_rules":[]}' \
...
data = {
    ...
    'category_keyword_rules': json.dumps({
        'positive_rules': [
            {
                'group_name': 'Invoice keywords',
                'min_hit': 2,
                'words': ['invoice code', 'invoice number'],
            }
        ],
        'negative_rules': [],
    }),
...
- `category_keyword_rules` (optional): Keyword rules for classification (JSON string). Positive rules are used for matching, while negative rules are used for filtering. You can configure up to 10 positive rule groups and 1 negative rule group, with up to 20 keywords in each group

<Note>
  A rule group matches when at least `min_hit` entries in `words` match. Positive rule groups are combined with OR; matching a negative rule group excludes the category. On creation, omitting this parameter or passing two empty arrays leaves keyword rules unconfigured.
</Note>
...
if result.get("code") == 200:
    ...
    for category in categories:
        ...
        print(f"Category keyword rules: {category.get('category_keyword_rules')}")
else:
...
For categories with keyword rules configured, `category_keyword_rules` contains the complete rule object for display or later updates.
      ...
      "positive_rules": [
        {
          "group_name": "Invoice keywords",
          "min_hit": 1,
          "words": ["VAT invoice", "invoice number"]
        }
      ],
      "negative_rules": []
    },
...
- `category_prompt` (optional): Prompt for classification, max length 500
- `category_keyword_rules` (optional): Keyword rules for classification. Positive rules are used for matching, while negative rules are used for filtering. You can configure up to 10 positive rule groups and 1 negative rule group, with up to 20 keywords in each group. Omit it or pass `null` to keep existing rules, pass a valid object to replace all rules, or pass `{"positive_rules":[],"negative_rules":[]}` to clear them
