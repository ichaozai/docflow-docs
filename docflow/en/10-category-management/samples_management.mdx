...
<Tip>
  Sample files are example documents for file categories, used to train and optimize document classification and extraction models. Each file category requires at least one sample file, with a maximum of 20 sample files. This guide introduces how to manage sample files for file categories via API.
</Tip>
...
- `files` (required): Sample files; a single request can contain at most 20 files, and the category total after upload cannot exceed 20, so the actual upload capacity is `20 - current sample count`
...
<Note>
  **Sample file requirements**:
  ...
  - Each category can retain at most 20 samples; subtract the existing sample count when adding more
