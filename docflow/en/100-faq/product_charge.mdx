...
You can visit "[Enterprise Document Processing Agent Platform - DocFlow Pricing](https://www.textin.com/product/textin_docflow#:~:text=DocFlow%20%E5%AE%9A%E4%BB%B7)" on the TextIn official website to view detailed product pricing information.From there, you can top up **T-Coins** to meet your usage needs.

## 02 **Pricing Inquiry**

During usage, you can clearly view T-coin order records and consumption details in the "[Top-up and Consumption Details](https://www.textin.com/console/dashboard/userCenter/finance)" section of your workspace dashboard, allowing you to keep track of your account's financial dynamics at any time.
...
(1) When using file splitting and multi-image cropping functions, associated files are charged at 0.3 RMB per page.
...
**Exception: [Extract Specific Fields API](/docflow/en/06-extract/extract_specific_fields)** (`POST /api/app-api/sip/platform/v2/file/extract_fields`, used to extract additional fields or re-extract fields for tasks that have completed extraction) — each call is billed independently based on the number of pages of the task's document, at the "other file types" rate (0.3 RMB/page) regardless of file type. Billing occurs only when the extraction result is returned successfully; failed requests, failed extractions, or requests without any fields to extract are not billed. When the account balance is insufficient, this API returns error code `1005025075` (insufficient quota or billing check failed).
