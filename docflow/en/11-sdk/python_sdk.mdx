...
rules = {
    ...
    "negative_rules": [{
        "group_name": "Exclude receipts",
        "min_hit": 1,
        "words": ["receipt", "payment receipt"],
    }],
}

client.category.create(
    workspace_id="123", name="VAT invoice", extract_model=ExtractModel.Model_1,
    sample_files=["/path/to/invoice.pdf"], fields=[{"name": "Invoice number"}],
    category_keyword_rules=rules,
)

# On update, send dual empty arrays to explicitly clear configured rules.
...
- On update, omit it or pass `None` for no change; a valid object replaces all rules; only dual empty arrays clear them. `{}` and blank strings are invalid.
- In the SDK, a rule group's `group_name` is optional: positive groups default to `group_1`, `group_2`, and so on, while a negative group defaults to `exclude`. It is display/tracking metadata only and does not affect matching or conflict detection; names returned by the API are preserved on a subsequent update.
- Keyword comparison ignores case and all whitespace. A normalized keyword must be non-empty, unique within its group, and at most 50 characters. A group has at most 20 keywords; a category has at most 10 positive and 1 negative group; `min_hit` must be from 1 to the number of group keywords.
- Positive and negative rules in the same category cannot share a keyword. Other enabled categories in the workspace cannot have a duplicate same-type rule group. The SDK pre-validates rules it can determine locally; the service performs final validation, and a cross-category conflict returns `1005015019`.
