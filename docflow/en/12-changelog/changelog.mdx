...
- `GET /api/app-api/sip/platform/v2/category/list`: Added optional `exclude_unadded_preset_categories` to exclude preset categories not added to the workspace. Defaults to `false`, preserves existing behavior, and works together with `enabled`.
...
- See [Product Pricing](/docflow/en/100-faq/product_charge) for details.
...
- [Create File Category](/api-reference/file-category/create-file-category) and [Update File Category](/api-reference/file-category/update-file-category) now support `category_keyword_rules`. Positive rules match a category, while negative rules filter it out.
- [List File Categories](/api-reference/file-category/list-file-categories) now returns `category_keyword_rules` when configured.
