---
title: "タスクステータス照会"
description: "タスクステータス、失敗理由、処理時間などの照会方法"
---

<Tip>
  このページでは、DocFlow タスクの処理ステータス、失敗理由、処理時間を照会する方法を説明します。タスクステータスを確認することで、ファイル処理の進行状況と結果を把握できます。
</Tip>

DocFlow は非同期処理モードを使用します。ファイルアップロード後、ファイルは処理キューに入ります。複数の方法でタスクステータスを照会し、処理進行状況、失敗理由、処理時間を確認できます。
...
`batch_number` を使用して、バッチ全体のタスクステータスを照会します。
...
`file_id` を使用して、特定ファイルのタスクステータスを照会します。
...
`task_id` を使用して、特定タスクのステータスを照会します。
