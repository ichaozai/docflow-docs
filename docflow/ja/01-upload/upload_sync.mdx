...
- **通常アップロード API**: ファイルアップロード後すぐに返ります。処理結果は後続で `/file/fetch` API から照会する必要があります
- **同期アップロード API**: ファイルアップロード後、処理完了まで待機します。追加照会なしで完全な処理結果を直接返します
...
<Info>
  同期アップロードに適さない場面では、通常アップロード API `/file/upload` と照会 API `/file/fetch` を組み合わせて、非同期処理フローを実装することをおすすめします。
</Info>
