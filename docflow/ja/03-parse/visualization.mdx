...
出力ディレクトリで、テキストボックスの描画効果を確認できます：

![visualization](../images/page_1_boxed.png)

## サンプルコードのロジック

- **解析結果を取得**：結果取得インターフェースを `with_document=true` を有効にして呼び出し、`result.files[].document` を取得します。
...
- **座標スケーリング**：`scale_x = img_width / page.width`、`scale_y = img_height / page.height` を計算し、返された座標をベース画像のピクセル座標系に比例してスケーリングします。
...
- **オプションの拡張**：
  ...
  - 文字レベルのハイライト：`charPositions` が返された場合、各文字座標を比例スケーリングして描画することで、より精密なエコー効果が得られます。
  - パフォーマンス最適化：一括描画、解像度のトレードオフ、オンデマンドページレンダリングにより、一度に多くのページを処理してラグが発生するのを回避します。
  - 堅牢性：null 値のチェック、座標境界のクリッピング、ネットワーク/解析例外のフォールトトレランス。
