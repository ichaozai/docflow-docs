...
業務要件が分類結果のみを必要とする場合、アップロードインターフェースに `target_process=classify` パラメータを追加することで、分類完了後にワークフローを終了し、抽出処理をスキップできます。
...
`file/fetch` インターフェースを使用して分類結果を照会します：
...
`target_process=classify` を使用して分類のみを行う場合、`recognition_status` フィールドは以下のようにステータスが遷移します：
