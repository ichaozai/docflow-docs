...
- ファイル照会 API（`/api/app-api/sip/platform/v2/file/fetch`）でファイル情報を取得し、その中の `task_id` を抽出タスク ID として使用します
...
6. **非同期実行**: レビュータスクは非同期で実行されます。タスク作成後は、[レビュー結果を取得](./get_result) API でレビューステータスと結果を照会してください
