...
curl -X POST \
  ...
  -F 'category_keyword_rules={"positive_rules":[{"group_name":"インボイスキーワード","min_hit":2,"words":["インボイスコード","インボイス番号"]}],"negative_rules":[]}' \
...
data = {
    ...
    'category_keyword_rules': json.dumps({
        'positive_rules': [
            {
                'group_name': 'インボイスキーワード',
                'min_hit': 2,
                'words': ['インボイスコード', 'インボイス番号'],
            }
        ],
        'negative_rules': [],
    }, ensure_ascii=False),
...
- `category_keyword_rules`（任意）: 分類に使用するキーワードルール（JSON 文字列）。正方向ルールは一致判定、反方向ルールは除外判定に使用します。正方向ルールは最大 10 グループ、反方向ルールは最大 1 グループで、各グループには最大 20 個のキーワードを設定できます

<Note>
  同一ルールグループ内で `words` のうち `min_hit` 件以上が一致すると、そのグループが一致します。正方向ルールグループは OR 条件で、反方向ルールグループに一致するとカテゴリから除外されます。作成時に省略するか、両方に空配列を渡すと、キーワードルールは設定されません。
</Note>
...
キーワードルールが設定されているカテゴリでは、`category_keyword_rules` に完全なルールオブジェクトが返され、表示や後続の更新に利用できます。
...
- `category_prompt`（任意）: 分類用プロンプト。最大 500 文字
- `category_keyword_rules`（任意）: 分類に使用するキーワードルール。正方向ルールは一致判定、反方向ルールは除外判定に使用します。正方向ルールは最大 10 グループ、反方向ルールは最大 1 グループで、各グループには最大 20 個のキーワードを設定できます。省略または `null` で既存ルールを維持し、有効なオブジェクトで全ルールを置換します。`{"positive_rules":[],"negative_rules":[]}` を渡すとルールをクリアします
