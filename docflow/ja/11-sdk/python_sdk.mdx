...
- キーワード比較では大文字・小文字および各種空白を無視します。正規化後のキーワードは空、グループ内重複、50 文字超過にできません。1 グループは最大 20 キーワード、カテゴリは正方向最大 10 グループ・反方向最大 1 グループで、`min_hit` は 1 からグループ内キーワード数までです。
- 同じカテゴリの正方向と反方向のルールに同じキーワードを含めることはできません。同じワークスペースの他の有効なカテゴリに同種の重複ルールグループも設定できません。SDK はローカルで判断できるルールを事前検証し、サービスが最終検証を行います。カテゴリ間競合のエラーコードは `1005015019` です。
