...
カテゴリを作成または更新するときは、`category_keyword_rules` で分類キーワードルールを設定できます。正方向ルールグループは OR 条件で、反方向ルールグループに一致した場合はカテゴリを除外します。

SDK は既定でワークスペース内の有効なカテゴリをページング取得し、リクエスト前にカテゴリ間の同種ルールグループ競合を事前検証します。追加リクエストを省略するには `check_keyword_rule_conflicts=False` を指定できますが、サーバーでは常に最終的な競合検証が行われます。
...
client.category.create(
    workspace_id="123", name="VAT インボイス", extract_model=ExtractModel.Model_1,
    sample_files=["/path/to/invoice.pdf"], fields=[{"name": "請求書番号"}],
    category_keyword_rules=rules,
)

# 更新時は両方の空配列を渡して、設定済みルールを明示的にクリアします。
...
- 作成時にこのパラメータを省略するとルールは未設定になります。両方の空配列も同じ意味です。
- 更新時に省略または `None` を渡すと変更しません。有効なオブジェクトは全置換し、両方の空配列だけがクリアを意味します。`{}` と空白文字列は無効です。
- SDK ではルールグループの `group_name` は省略可能です。正方向グループには順に `group_1`、`group_2`…、反方向グループには `exclude` が自動設定されます。これは表示・追跡用の情報で、分類の一致判定や競合判定には影響しません。API から取得した名称は、その後の更新時にも保持されます。
- キーワードの大文字・小文字と空白は入力どおり保持されます。各キーワードは空または 50 文字超過にできず、同一ルールグループ内に元の文字列が完全に同じキーワードを重複して設定できません。1 グループは最大 20 キーワード、カテゴリは正方向最大 10 グループ・反方向最大 1 グループで、`min_hit` は 1 からグループ内キーワード数までです。
- 同じカテゴリの正方向と反方向のルールに、元の文字列が完全に同じキーワードを含めることはできません。同じワークスペースの他の有効なカテゴリに、元のキーワード集合が同じ同種ルールグループも設定できません。SDK はローカルで判断できるルールを事前検証し、サービスが最終検証を行います。カテゴリ間競合のエラーコードは `1005015019` です。
